...
def fitness(x):
	...
	for i in range(Tmonth):
		p = (g * ita_S3 * R3[i] * H3[i]) / 1000 if (g * ita_S3 * R3[i] * H3[i]) / 1000 <= power3 else power3
		z = 1 - (p / power3)
		F = F + z
	return F
...
def E3(x):
	...
	for i in range(Tmonth):
		p = g * ita_S3 * R3[i] * H3[i] / 1000 if (g * ita_S3 * R3[i] * H3[i] / 1000) <= power3 else power3
		e3[i] = (p * Days[i] * 24) / 1000
		Q3[i] = (p / (g * ita_S3 * H3[i]) * 1000)
...
Elevation_for_Sunkoshi_3 = Height3(xopt)
...
Energy_Sunkoshi_3, Discharge_for_Sunkoshi_3, Spill_for_Sunkoshi_3 = E3(xopt)
